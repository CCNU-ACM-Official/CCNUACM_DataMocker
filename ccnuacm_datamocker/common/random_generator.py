...
class RandomGenerator:
    ...
    def randint(self, *ranges) -> int:
        ...
        if leftmost >= rightmost:
            raise ValueError(f"Invalid range, leftmost=`{leftmost}`, rightmost=`{rightmost}`.")
        return self._random_generator.randint(leftmost, rightmost)
